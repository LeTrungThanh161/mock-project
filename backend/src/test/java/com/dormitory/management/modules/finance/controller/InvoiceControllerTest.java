...
class InvoiceControllerTest {
    ...
    void getMyInvoicesReturnsInvoicesForCurrentStudent() {
        ...
        InvoiceController controller = new InvoiceController(invoiceRepository, studentRepository, contractRepository, null);
        ...
        when(studentRepository.findByAccountId(7)).thenReturn(Optional.of(student));

        Room room = new Room();
        room.setRoomId(20);
        Contract contract = new Contract();
        contract.setRoom(room);
        when(contractRepository.findByStudent_StudentId(10)).thenReturn(List.of(contract));
        ...
        Invoice invoice = Invoice.builder()
                ...
                .paymentStatus(PaymentStatus.Unpaidgit branch)
                ...
                .build();
        when(invoiceRepository.findByRoom_RoomIdIn(List.of(20))).thenReturn(List.of(invoice));

        ResponseEntity<List<Invoice>> response = controller.getMyInvoices(request);
    ...
    void getInvoicesReturnsBuildingScopedInvoicesForManager() {
        ...
        Invoice invoice = Invoice.builder().invoiceId(200).build();
        when(invoiceRepository.findByBuilding_BuildingId(3)).thenReturn(List.of(invoice));

        ResponseEntity<List<Invoice>> response = controller.getInvoices(request);
